...
<xsl:variable name="mappings" select="document('../../models/domain/WebStores/analysis/mapping/WebStoresLayoutStateMappingModel.xml')/mapping"/>
		...
		<xsl:element name="screen">
			...
			<xsl:attribute name="layout">
				<xsl:if test="$mappings/screen[@name=$screenName]">
					<xsl:value-of select="$mappings/screen[@name=$screenName]/@layout"/>
				</xsl:if>
			</xsl:attribute>
	...
	<xsl:element name="element">
		...
		<xsl:attribute name="at">
			<xsl:if test="$mappings/element[@name=$elementName]">
				<xsl:value-of select="$mappings/element[@name=$elementName]/@at"/>
			</xsl:if>
		</xsl:attribute>
